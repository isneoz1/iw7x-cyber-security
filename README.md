...
**iw7x** brings **28,376** offensive & defensive security tools — from Nmap to BloodHound, Metasploit to Volatility — into a single command‑line arsenal that **installs, updates and launches** any of them for you. Built for Kali Linux. Free forever.
...
[![Tools](https://img.shields.io/badge/Tools-28%2C376-FF47B3?style=for-the-badge&labelColor=1a1a2e)](catalog.json)
[![Categories](https://img.shields.io/badge/Categories-52-9652FF?style=for-the-badge&labelColor=1a1a2e)](#the-arsenal--52-categories-28376-tools)
...
<img src="assets/iw7x-preview.svg" alt="iw7x — cybersecurity arsenal: 28,376 tools across 52 categories, built for Kali Linux, by NeoZ" width="840">
...
| | |
|---|---|
| **The whole ecosystem** | **28,376 tools** across **52 categories** — OSINT, web, wireless, exploitation, forensics, reversing, crypto, cloud, Active Directory, mobile, IoT, CTF, honeypots, threat hunting, blue team and far more. |
...
## The Arsenal — 52 categories, 28,376 tools
...
It also indexes **GitHub's own security topics** (`hacking-tool`, `osint`, `bug-bounty`, `red-team`, `reverse-engineering`, `ctf`, `malware-analysis`…), keeping the most‑starred real tools.

The goal is simple: **every security tool on Earth, always current, in one place.**

### Go deeper — optional GitHub token

Without a token the GitHub crawl stays gentle (rate‑limited). Give iw7x a **read‑only** token and it goes far deeper and faster — thousands more tools.

1. Create one at **[github.com/settings/tokens](https://github.com/settings/tokens)** → *Tokens (classic)* → **Generate** with **no scopes ticked** (public search needs none, so the token can read nothing private — it's safe).
2. Expose it to iw7x as an **environment variable** — never in the code, never committed:

   **Windows (PowerShell), once:**
   ```powershell
   setx IW7X_GITHUB_TOKEN "ghp_your_token_here"
   ```
   Then reopen your terminal.

   **Linux / macOS** — add to `~/.bashrc` or `~/.zshrc`:
   ```bash
   export IW7X_GITHUB_TOKEN="ghp_your_token_here"
   ```

iw7x reads `IW7X_GITHUB_TOKEN` (or `GITHUB_TOKEN` / `GH_TOKEN`) automatically on the next launch. A **user‑scoped** environment variable is private to your account: other users of the machine can't read it, and it never touches the repository. If a token ever leaks, revoke it on that same page and generate a new one.
